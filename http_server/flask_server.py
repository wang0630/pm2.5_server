...
def get_recent_data(pos):
	try:
		...
		return_data = list(target_data.find({'position' : pos}).sort([('_id', -1)]).limit(1)) # sort data backwards, gets the newest data
		result = {'position' : return_data[0].get('position'), 'date' : return_data[0].get('date'), 'temperature' : return_data[0].get('temp'),
			'humidity' : return_data[0].get('humidity'), 'pm10' : return_data[0].get('pm10'), 'pm25' : return_data[0].get('pm25'),
